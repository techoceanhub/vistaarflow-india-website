      ...
      <article class="business-card reveal">
        ...
        <div class="art-labels catalog-labels">
          <span>Property</span><span>Course</span><span>Product</span><span>Vehicle</span><span>Service</span><span>Custom
            Catalog</span><span>Payment Gateway</span></div>
      </article>
      <article class="business-card reveal">
        ...
        foreach ($plans as $i => $p): ?>
          <article class="price-card <?php echo $i === 1 ? 'featured' : ''; ?> reveal"><?php if ($i === 1): ?><span
            ...
            <div class="price"><small>₹</small><strong><?php echo esc_html($p[2]); ?></strong><span>/ month</span></div><a
              class="button <?php echo $i === 1 ? '' : 'button-outline'; ?>" href="https://app.vistaarflow.in/signup">Choose
      ...
      <div class="post-grid">
        ...
        if ($vf_posts->have_posts()):
          while ($vf_posts->have_posts()):
            $vf_posts->the_post(); ?>
            <article class="post-card reveal"><a class="post-thumb"
                href="<?php the_permalink(); ?>"><?php if (has_post_thumbnail()) {
                    the_post_thumbnail('large');
                  } else { ?><span
                    class="post-placeholder">V</span><?php } ?><span
                  class="post-category"><?php $cats = get_the_category();
    ...
    <div class="container contact-layout reveal">
      ...
      <div class="contact-visual" aria-hidden="true">
        <div class="contact-card">
          ...
          <p>Sales, support, onboarding, or general questions—one inbox, one team.</p>
          <div class="contact-card-email">contact@vistaarflow.in</div>
        </div>
      </div>
    </div>
  </section>
